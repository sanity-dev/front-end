...
@Component({
  ...
  template: `
    ...
    <!-- Sheet -->
    <div @slideUp class="fixed bottom-0 left-0 right-0 z-50 bg-white rounded-t-3xl shadow-2xl max-h-[93vh] flex flex-col">
      ...
      <!-- Header -->
      <div class="px-6 pt-2 pb-4 border-b border-[#D9D9D9]/50 flex items-center justify-between flex-shrink-0">
        <div>
      ...
      <div class="px-6 py-3 flex items-center gap-1 flex-shrink-0">
        <ng-container *ngFor="let s of [1,2,3]; let i = index">
          ...
          <div *ngIf="i < 2" class="h-0.5 w-8 rounded transition-all duration-300"
            [style.background]="step > i+1 ? '#4CA1AF' : '#D9D9D9'"></div>
        </ng-container>
        <span class="ml-auto text-xs text-gray-400">Paso {{ step }} de 3</span>
      </div>

      <!-- Form -->
      <form [formGroup]="form" (ngSubmit)="onSubmit()" class="flex-1 overflow-y-auto px-6 pb-8 scrollbar-hide">
        ...
        <div *ngIf="step === 1" class="space-y-5 py-4">
          ...
          <div>
            <label class="block text-sm font-semibold text-[#1d1d1d] mb-1.5">Título profesional *</label>
            ...
            <p *ngIf="f['tituloProfesional'].invalid && f['tituloProfesional'].touched"
              class="text-red-400 text-xs mt-1">Campo requerido (mínimo 5 caracteres)</p>
          </div>
          ...
          <div>
            ...
            <textarea formControlName="presentacion" rows="4"
              ...
              class="w-full rounded-xl border border-[#D9D9D9] bg-gray-50 px-4 py-3 text-sm text-[#1d1d1d]
                     resize-none focus:outline-none focus:ring-2 focus:ring-[#4C9EEB] focus:border-transparent transition"></textarea>
            <div class="flex justify-between mt-1">
              <p *ngIf="f['presentacion'].invalid && f['presentacion'].touched" class="text-red-400 text-xs">
        ...
        <div *ngIf="step === 2" class="space-y-5 py-4">
          ...
          <div>
            ...
            <p *ngIf="submitted && getArray('especialidades').length === 0"
              class="text-red-400 text-xs mt-1">Selecciona al menos una</p>
          </div>

          <!-- Servicios -->
          <div>
            <label class="block text-sm font-semibold text-[#1d1d1d] mb-2">Servicios que ofreces *</label>
            <div class="flex flex-wrap gap-2">
            ...
            <p *ngIf="submitted && getArray('servicios').length === 0"
              class="text-red-400 text-xs mt-1">Selecciona al menos uno</p>
          </div>
          ...
          <div class="space-y-3">
            <div *ngFor="let slot of disponibilidadSlots; let i = index"
              ...
              <div class="flex items-center gap-1.5 mb-3 flex-wrap">
                <button type="button" *ngFor="let dia of dias"
                  (click)="slot.dia = dia"
              ...
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <label class="text-[11px] font-semibold text-gray-500 block mb-1">Hora inicio</label>
                  <input type="time"
...
export class SpecialistFormToastComponent implements OnInit {
  ...
  especialidadesOpciones = [
    'Psicología clínica', 'Psicología cognitivo-conductual',
    'Terapia familiar y de pareja', 'Psicoterapia gestáltica',
    'Neuropsicología', 'Psicología infantil y adolescente',
    'Psicoanálisis', 'Mindfulness y bienestar',
    'Terapia de duelo', 'Manejo del estrés y ansiedad',
  ];

  serviciosOpciones = [
    'Consulta individual', 'Terapia de pareja',
    'Terapia familiar', 'Terapia grupal',
    'Evaluación psicológica', 'Orientación vocacional',
    'Sesión de emergencia', 'Seguimiento mensual',
  ];
  ...
  ngOnInit(): void {
    this.form = this.fb.group({
      ...
      presentacion:      ['', [Validators.required, Validators.minLength(30), Validators.maxLength(600)]],
      especialidades:    [[]],
      servicios:         [[]],
    });
    this.addSlot();
