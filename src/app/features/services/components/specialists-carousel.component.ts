...
@Component({
  ...
  template: `
    <div class="pb-28">
      <!-- Header -->
      <div class="mb-5">
        <p class="text-[10px] font-bold uppercase tracking-widest text-secondary-background mb-1">
          Directorio
        </p>
      ...
      <div *ngIf="!filteredList.length" class="flex flex-col items-center py-16 text-center">
        <div
          class="w-20 h-20 rounded-full bg-gray-100 flex items-center justify-center text-4xl mb-4"
        >
          🔍
        </div>
      ...
      <div class="space-y-4">
        <div
          *ngFor="let s of filteredList; trackBy: trackById"
          class="bg-white rounded-3xl border border-white-sanity/60 overflow-hidden"
        >
          <!-- Top -->
          <div
            class="p-5 flex items-start gap-4 bg-gradient-to-r from-secondary-background/5 to-third-background/5"
          >
            <!-- Foto de perfil -->
            <div
            ...
            <div class="flex-1 min-w-0">
              <h3 class="font-bold text-text-primary truncate">
                {{ s.nombre || s.email || '' | uppercase }}
              </h3>
              <p class="text-xs font-semibold text-third-background truncate mt-0.5">
                {{ s.tituloProfesional }}
              </p>
              <div class="flex flex-wrap gap-1 mt-2">
                <span
                  *ngFor="let esp of s.especialidades.slice(0, 2)"
                  class="px-2 py-0.5 rounded-md text-[10px] font-bold bg-third-background/10 text-third-background"
                >
                  {{ esp }}
                </span>
                <span
                  *ngIf="s.especialidades.length > 2"
          ...
          <div class="px-5 py-3 border-b border-white-sanity/40">
            ...
            <div class="flex flex-wrap gap-1">
              <span
                *ngFor="let srv of s.servicios.slice(0, 3)"
...
export class SpecialistsCarouselComponent {
  ...
  getSlots(s: Specialist) {
    return parseDisponibilidad(s.disponibilidad ?? '[]');
  }
  trackById(_: number, s: Specialist) {
    return s.userId;
  }
}
